...
void ProtocolTree::setup(Protocol protocol)
{
	...
	for (it_category = categorys.begin(); it_category != categorys.end(); ++it_category)
	{
		QTreeWidgetItem *catetory_item = addChildItem(protocol_item, it_category->text().c_str(), it_category->tip().c_str());
		...
		{
			QTreeWidgetItem *field_item = addChildItem(catetory_item, it_field->text().c_str(), it_field->tip().c_str());
			setItemWidget (field_item, 1, new QLineEdit(this) );
		}
	}
}
